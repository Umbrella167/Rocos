...
public:
    ...
    double theta(const CVector &v)
    {
        ...
        if (_theta > 3.14159265358979323846)
            return _theta - 2 * 3.14159265358979323846;
        if (_theta < -3.14159265358979323846)
            return _theta + 2 * 3.14159265358979323846;
        return _theta;
    }
...
public:
    ...
    {
        return ((this->point1().x() == rhs.point1().x()) && (this->point1().y() == rhs.point1().y()) && (this->point2().x() == rhs.point2().x()) && (this->point2().y() == rhs.point2().y()));
    }
...
public:
    ...
    CGeoPoint segmentsIntersectPoint(const CGeoSegment &p) const
    {
        ...
        if (IsPointOnLineOnSegment(interPoint) && p.IsPointOnLineOnSegment(interPoint))
            ...
            return CGeoPoint(9999, 9999);
    }
    double dist2Point(const CGeoPoint &p)
    {
        CGeoPoint tmpProj = projection(p);
        if (IsPointOnLineOnSegment(tmpProj))
            return p.dist(tmpProj);
        else
            return std::min(_start.dist(p), _end.dist(p));
    }
    double dist2Segment(const CGeoSegment &s)
    {
        if (IsSegmentsIntersect(s))
            return 0;
        else
            return std::min(dist2Point(s.point1()), dist2Point(s.point2()));
    }
