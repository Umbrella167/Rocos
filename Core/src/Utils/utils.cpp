...
namespace Utils
{
     ...
     * 判断防守是否可能抓住球
     * @param  {} undefined :
     * @return 返回几号后卫可以抓住球，返回-1表示没有
     */

    int DefenderTryToCatchBall()
    {
        double ball_pos[2] = {Tick[now].ball_pos.x(), Tick[now].ball_pos.y()};
        double player_pos[2][2] = {{Tick[now].our.defend_player_num[0].x(), Tick[now].our.defend_player_num[0].y()},
                                   {Tick[now].our.defend_player_num[1].x(), Tick[now].our.defend_player_num[1].y()}}; // FIXME:修一下输入的

        for (int i = 0; i < 2; i++)
        {
            if (ball_pos[i] * ball_pos[i] + player_pos[i][0] * player_pos[i][0] < 1000000.0)
            {
                return i;
            }
        }
