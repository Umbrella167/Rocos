...
if DEBUG_MODE then
    ...
    DEFENDER_DEBUG_POSITION_Y = 3520
...
gPlayTable.CreatePlay {
    ...
    ["init"] = {
        switch = function()
            ...
            if bufcnt(player.toPointDist('Defender', INITPOS_DEFENDER) < 100, 35) and bufcnt(player.toPointDist('Tier', INITPOS_TIER) < 100, 35) then
                return "defense"
            end
        end,
    ...
    ["defense"] = {
        switch = function()
            ...
            if bufcnt(player.toBallDist('Defender') > 5000, 140) then
                return "init"
            end
        end,
