...
-- Center support position (central, ready for rebounds)
local center_pos = function()
  return CGeoPoint(2000, 0)
end
...
gPlayTable.CreatePlay {
  ...
  -- Ball is placed at corner. All robots move into position.
  -- Assister moves to ball, Kicker/Special run to attack positions.
  ["ready"] = {
    switch = function()
    ...
    Center = task.goCmuRush(center_pos, toBallDir("Center")),
    Defender = defenderDirectTask,
    Goalie = gSubPlay.roleTask("Goalie", "Goalie"),
    match = "[AKSC]{DG}"
  },

  -- Assister passes toward Kicker's current position (dynamic)
  -- Indirect free kick: first touch must be a pass (cannot score directly)
  ["pass"] = {
    switch = function()
  ...
  -- Kicker intercepts the pass and shoots at goal
  ["attack"] = {
    switch = function()
